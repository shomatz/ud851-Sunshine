...
public class MainActivity extends AppCompatActivity {
    ...
    public class NetworkRequest extends AsyncTask<String, Void, String[]>{
        ...
        protected String[] doInBackground(String... urls) {
            ...
            URL buildUrl = NetworkUtils.buildUrl(search);
            try {
                String jsonRequest = NetworkUtils.getResponseFromHttpUrl(buildUrl);
                String[] requests = OpenWeatherJsonUtils.getSimpleWeatherStringsFromJson(MainActivity.this, jsonRequest);
                return requests;
            } catch (Exception e) {
                e.printStackTrace();
                return null;
        ...
        @Override
        protected void onPostExecute(String[] requests) {
            if(requests.length != 0) {
                for (String request : requests) {
                    mWeatherTextView.append(request + "\n\n\n");
                }
            }
        }
    }
